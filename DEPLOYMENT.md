...
Railway создаст базу данных PostgreSQL (сервис `Postgres` появится на канвасе).

### Шаг 4 — Добавить переменные окружения

Перейти в сервис **GDEMOYJOB2** → вкладка **Variables** → **New Variable**.

Добавить обе переменные:
...
> `${{Postgres.DATABASE_URL}}` — Railway-синтаксис для ссылки на переменную соседнего сервиса.  
> Railway подставит реальный URL автоматически и передеплоит бота.  
> Таблицы (`users`, `resumes`, `search_history`) создадутся при первом старте бота.
...
| `DATABASE_URL` | ✅ | URL базы данных | см. ниже |
...
```
Railway подставит реальный URL вида `postgresql://user:pass@host:5432/dbname`.

> Приложение автоматически преобразует `postgresql://` → `postgresql+asyncpg://` для работы с async SQLAlchemy (`config.py`, функция `_db_url`).
